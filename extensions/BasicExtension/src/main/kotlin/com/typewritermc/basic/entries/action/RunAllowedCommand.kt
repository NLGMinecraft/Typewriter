...
class ConsoleCommandActionEntry(
    ...
    @Placeholder
    @MultiLine
    @Help("Every line is a different command. Commands should not be prefixed with <code>/</code>. Only allowed commands by NLG")
    private val command: Var<String> = ConstVar(""),
) : ActionEntry {
    override fun ActionTrigger.execute() {
        val allowedCommands = listOf("bank %player_name%", "plots open %player_name%")
        ...
        Dispatchers.Sync.launch {
            ...
            for (cmd in commands) {
                ...
                if (allowedCommands.none { allowed -> cmd.startsWith(allowed.removeSuffix("%player_name%").trim()) }) {
                    continue
                }
